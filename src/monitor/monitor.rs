...
impl Monitor {
    ...
    pub async fn run_with_reconnect(
        ...
        app_info!("Monitor starting with auto-reconnect capability");

        loop {
            // 检查取消信号
            if cancellation_token.is_cancelled() {
                app_info!("Monitor cancelled before attempting connection");
                break;
            }
            ...
            if reconnect_count > 0 {
                ...
                tokio::select! {
                    _ = cancellation_token.cancelled() => {
                        app_info!("Monitor cancelled during reconnection backoff");
                        break;
                    }
            ...
            match self.run_single_connection(cancellation_token.clone()).await {
                Ok(_) => {
                    app_info!("Monitor connection ended gracefully");
                    break;
                }
                Err(e) => {
                    ...
                    app_error!("Monitor connection error:{:?}", e);
                    // todo: 分类错误类型
                    ()
                }
            }
        ...
        app_info!("Monitor stopped gracefully");
        Ok(())
    }
    ...
    async fn run_single_connection(
        ...
        app_info!("Monitor subscription established, processing blocks");
        ...
        let connection_monitor = tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                app_debug!("🔗 gRPC connection monitor: stream active");
            }
        });
        ...
        loop {
            tokio::select! {
                _ = cancellation_token.cancelled() => {
                    app_info!("Monitor processing cancelled");
                ...
                data = stream.next() => {
                    match data {
                                ...
                                match update {
                                    UpdateOneof::Ping(_) => {
                                        let _ = subscribe_tx
                                            ...
                                            .await;
                                        app_debug!("service is ping: {:#?}", Local::now());
                                    }
                                    UpdateOneof::Pong(_) => {
                                        app_debug!("service is pong: {:#?}", Local::now());
                                    }
                                    UpdateOneof::Block(sub_block) => {
                                        let slot = sub_block.slot;
                                        app_info!("📥 收到 Slot {}", slot);

                                        let message_queue = Arc::clone(&self.message_queue);
                                        // 这里可能要添加线程控制
                                        tokio::spawn(async move {
                                            let _permit = match BLOCK_SEMAPHORE.acquire().await {
                                                Ok(p) => p,
                                                Err(e) => {
                                                    app_error!("Failed to acquire semaphore for slot {}:{}", slot, e);
                                                    return;
                                                }
                                            };
                                            ...
                                            if let Err(e) = Self::process_block_static(sub_block, message_queue).await {
                                                app_error!("Failed to process block {}: {}", slot, e);
                                            }

                                            let elapsed = start.elapsed();
                                            app_info!("🕐 Slot {} 总耗时: {:?}", slot, elapsed);
                                        });
                                    }
                                    _ => {}
                                }
                        ...
                        Some(Err(e)) => {
                            app_error!("Stream error: {:?}", e);
                            connection_monitor.abort();
                            return Err(GrpcError::StreamError(format!("Stream error: {}", e)).into());
                        }
                        None => {
                            app_warn!("Monitor stream ended unexpectedly");
                            connection_monitor.abort();
    ...
    fn process_transaction(
        ...
        app_debug!("slot:{}, sig:{:?}", block_slot, sig);
...
mod tests {
    ...
    async fn test_monitor() {
        ...
        // 创建消息队列
        let kafka_config = KafkaQueueConfig::default();
        let message_queue = KafkaMessageQueue::new(kafka_config).unwrap();
