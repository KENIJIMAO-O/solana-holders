...
mod tests{
    ...
    use fixnum::{FixedPoint, typenum::U12, typenum};
    ...
    #[tokio::test]
    async fn test_click_house() -> Result<()> {
        // let client = Client::default().with_url("http://localhost:9000");
    ...
    async fn test_insert_event() {
        ...
        let event = Event {
            slot: 378204114,
            tx_sig: "3Wq8YFZEBR1rg7YNnB4XkSJpbL8QyXZwRYUL85zhCjSEZFCERsxm3rYzggBnih2C4dRRN71HiL8rFkZdys67h7gK".to_string(),
            mint_pubkey: "So11111111111111111111111111111111111111112".to_string(),
            account_pubkey: "Ascn1cbQCSXeKpAvXRbhqxKRnjh4o1ZSHTxZLQhgJQE5".to_string(),
            owner_pubkey: "7hKSRo8VLc7zHD3YK8WArZKGB3yHzDbR4LSxxGmRTvg9".to_string(),
        ...
        insert.write(&Event{
            slot: event.slot,
            tx_sig: event.tx_sig,
            mint_pubkey: event.mint_pubkey,
            account_pubkey: event.account_pubkey,
            owner_pubkey: event.owner_pubkey,
            delta: event.delta,
            confirmed: 0,
            _timestamp: Utc::now()
        }).await.unwrap();
